...
# Create and fit a GaussianNB classifier object
gnb = GaussianNB()
gnb.fit(X_train, Y_train)
prediction=gnb.predict(np.array((X_val)))
...
Submission_preds=gnb.predict(test)  #we containing the prediction of test in this variable
df=pd.DataFrame({"PassengerId":test_ids.values,
                                "Survived":Submission_preds,
                 })
df.to_csv("Submission1.csv",index=False )  #Transfering the above created data frame into new csv file(containing Survived and PassengerId)
